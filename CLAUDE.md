...
- **Account Map on Replit:** After `git pull`, **restart the server** (Stop → Run). On boot, the API logs `[account-map] runtime config` — confirm `pass1.maxSearches: 4`, `leadershipEnrichCap: 5`, and `pass2.searchesPerEntity: true` with `pass2.maxSearches: 5` before testing Zurich.
