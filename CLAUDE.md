...
- **Account Map on Replit:** After `git pull`, **restart the server** (Stop → Run). Set `MAPPING_MODEL=claude-haiku-4-5-20251001` for Haiku testing (recommended while tuning). On boot, the API logs `[account-map] runtime config` — confirm `pass1.maxSearches: 3`, `pass2.timeoutMs: 90000`, `leadershipEnrichCap: 5`, and `pass2.searchesPerEntity: true` before testing Zurich. Default model remains Sonnet in code; Haiku is env-only. Sonnet Pass 2 only: set `MAPPING_PASS_2_MODEL=claude-sonnet-4-6` with Haiku on Pass 1. Mapping no longer returns `groupBackground` — use Brief mode for deeper company context. Full walkthrough: [`docs/mapping-changes-explained.md`](docs/mapping-changes-explained.md).
