...
- **Account Map is cheap by default:** default model is **Haiku** (`claude-haiku-4-5-20251001`) and **structure-only** (single pass, no leadership enrichment). Opt into the expensive path via env: `MAPPING_MODEL=claude-sonnet-4-6` for Sonnet, `MAP_STRUCTURE_ONLY=0` to re-enable the Pass 2 leadership run. Sonnet Pass 2 only: `MAPPING_PASS_2_MODEL=claude-sonnet-4-6` with Haiku on Pass 1. Mapping no longer returns `groupBackground` — use Brief mode for deeper company context. Full walkthrough: [`docs/mapping-changes-explained.md`](docs/mapping-changes-explained.md).
- **Account Map on Replit:** After `git pull`, **restart the server** (Stop → Run). On boot, the API logs `[account-map] runtime config` — confirm `structureOnly: true` and `pass1Model: "claude-haiku-4-5-20251001"` unless you deliberately set the env overrides above.
- **Mapping route deadline:** the whole `/account-map` request is raced against `MAPPING_TIMEOUT_MS - 2s` with an `AbortController`; on deadline the server aborts in-flight SDK calls and returns 504 — it never hangs until the client abort.
